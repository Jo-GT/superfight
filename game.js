...
class Fighter {
  ...
  animate(dt) { this.frameClock += dt; this.stateTime += dt; const rate = this.state === 'idle' ? 115 : this.state === 'special' ? 42 : 82; if (this.frameClock > rate) { this.frameClock = 0; this.frame += 1; const frames = this.state === 'attack' && assets[this.kind].attackStages ? assets[this.kind].attackStages[this.attackStage - 1] : assets[this.kind][this.state] || assets[this.kind].idle; if (this.frame >= frames.length) { if (['attack','airAttack','power','special','dodge','hit'].includes(this.state)) this.setState('idle'); else this.frame = 0; } } }
