...
class Projectile {
  constructor(owner, type) { this.owner = owner; this.type = type; this.x = owner.x + owner.facing * 105; this.y = owner.y - (type === 'special' ? 135 : 100); this.direction = owner.facing; this.frame = 0; this.frameClock = 0; this.age = 0; this.hit = false; this.speed = type === 'special' ? 8.5 : type === 'attack4' ? 12 : 10; this.damage = type === 'special' ? 24 : type === 'attack4' ? 10 : type === 'attack1' ? 4 : 12; }
  update(dt, opponent) { this.age += dt; this.frameClock += dt; this.x += this.direction * this.speed * dt / 16; if (this.frameClock > 75) { this.frameClock = 0; this.frame += 1; } if (!this.hit && Math.abs(opponent.x - this.x) < (this.type === 'special' ? 115 : 75) && Math.abs(opponent.y - this.y) < 150) { opponent.takeHit(this.damage, this.direction * 34); this.owner.meter = Math.min(3, this.owner.meter + .6); if (this.type === 'attack1' || this.type === 'attack4') { this.owner.combo = Math.min(10, this.owner.combo + 1); this.owner.chainStage = this.owner.attackStage; } this.owner.comboTimer = 900; this.hit = true; } return this.age < (this.type === 'special' ? 1700 : 1100) && this.x > -250 && this.x < W + 250 && !this.hit; }
  draw() { const list = projectileFrames[this.type]; const path = assets.cyclops.projectileRoot + list[Math.min(this.frame, list.length - 1)]; const image = imageFor(path); if (!image.complete) return; const scale = this.type === 'special' ? 1.15 : this.type === 'attack4' ? .9 : .72; ctx.save(); ctx.translate(this.x, this.y); ctx.scale(this.direction * scale, scale); ctx.globalAlpha = this.type === 'special' ? .92 : 1; ctx.drawImage(image, -128, -128, 256, 256); ctx.restore(); }
}
...
class Fighter {
  ...
  update(dt, opponent) {
    ...
    if (this.state === 'attack' || this.state === 'power' || this.state === 'special') { this.checkStrike(opponent); const comboProjectile = this.state === 'attack' && (this.attackStage === 1 || this.attackStage === 4); if (this.kind === 'cyclops' && comboProjectile && !this.projectileSpawned && this.stateTime >= 230) { projectiles.push(new Projectile(this, this.attackStage === 4 ? 'attack4' : 'attack1')); this.projectileSpawned = true; } if (this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special') && !this.projectileSpawned && this.stateTime >= (this.state === 'special' ? 480 : 260)) { projectiles.push(new Projectile(this, this.state)); this.projectileSpawned = true; this.strikeConfirmed = true; } this.animate(dt); return; }
  ...
  checkStrike(opponent) { if (this.strikeConfirmed) return; const isCyclopsBeam = this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special'); if (isCyclopsBeam) return; const windows = this.state === 'attack' ? [140, 330] : this.state === 'power' ? [260, 560] : [480, 900]; const [start, end] = windows; if (this.stateTime < start || this.stateTime > end) return; const reach = this.state === 'special' ? 260 : this.state === 'power' ? 210 : 175; const damage = this.state === 'special' ? 24 : this.state === 'power' ? 12 : 6; if (Math.abs(opponent.x - this.x) < reach && Math.abs(opponent.y - this.y) < 180) { opponent.takeHit(damage, this.facing * 28); this.meter = Math.min(3, this.meter + .6); if (this.state === 'attack') { this.combo = Math.min(10, this.combo + 1); this.chainStage = this.attackStage; this.comboTimer = 900; } this.strikeConfirmed = true; } }
...
function updateHud() { document.querySelector('#p1-health').style.width = `${p1.health}%`; document.querySelector('#p2-health').style.width = `${p2.health}%`; document.querySelector('#p1-meter').style.width = `${p1.meter / 3 * 100}%`; document.querySelector('#p2-meter').style.width = `${p2.meter / 3 * 100}%`; document.querySelector('#p1-combo').textContent = `x${p1.combo}`; document.querySelector('#p2-combo').textContent = `x${p2.combo}`; }
...
function start() { reset(); projectiles = []; running = true; menuScreen.classList.add('hidden'); overlay.classList.add('hidden'); footerState.textContent = `ROUND ACTIVE // ${selectedDifficulty.toUpperCase()} CPU`; last = performance.now(); }
