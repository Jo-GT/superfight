...
class Fighter {
  ...
  playerInput(dt) { const left = keys.has('ArrowLeft'), right = keys.has('ArrowRight'), moving = left || right; if (pressed.has('ArrowDown')) { this.setState('dodge'); this.invulnerable = 300; return; } if (pressed.has('ArrowUp') && pressed.has('x')) { if (this.attack('risingAttack')) { this.y -= 2; return; } } if (pressed.has('x')) { if (this.attack(moving ? 'sprintAttack' : 'attack')) return; } if (pressed.has('a') && this.attack('power')) return; if (pressed.has('s') && this.attack('special')) return; if (moving) { this.x += (right ? 1 : -1) * 4.2 * dt / 16; this.setState('walk'); } else this.setState('idle'); if (pressed.has('ArrowUp')) { this.vy = -15; this.y -= 2; this.setState('jump'); } }
...
function start() { reset(); projectiles = []; paused = false; running = true; pauseOverlay.classList.add('hidden'); menuScreen.classList.add('hidden'); overlay.classList.add('hidden'); playMusic(fightMusic); footerState.textContent = `ROUND ACTIVE // ${selectedDifficulty.toUpperCase()} CPU`; last = performance.now(); }
