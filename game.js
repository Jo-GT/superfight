...
const p2MeterBar = document.querySelector('#p2-meter');
const p1ComboLabel = document.querySelector('#p1-combo');
const p2ComboLabel = document.querySelector('#p2-combo');
...
const runFrames = ['Sprint_00.png','Sprint_01.png','Sprint_02.png','Sprint_03.png','Sprint_04.png','Sprint_05.png','Sprint_06.png','Sprint_07.png'];
assets.cyclops.run = runFrames;
assets.wolverine.run = runFrames;
...
const kdFrames = ['Kd_00.png','Kd_01.png','Kd_02.png','Kd_03.png','Kd_04.png','Kd_05.png'];
const downgetupFrames = ['Downgetup_00.png','Downgetup_01.png','Downgetup_02.png','Downgetup_03.png'];
const landFrames = ['Land_00.png','Land_01.png','Land_02.png','Land_03.png'];
const sprintStartFrames = ['Sprintstart.png'];
const sprintEndFrames = ['Sprintend_00.png','Sprintend_01.png','Sprintend_02.png','Sprintend_03.png'];
const walkStartFrames = ['Walkstart.png'];
const walkEndFrames = ['Walkend.png'];
['cyclops', 'wolverine'].forEach(kind => { assets[kind].kd = kdFrames; assets[kind].downgetup = downgetupFrames; assets[kind].land = landFrames; assets[kind].runStart = sprintStartFrames; assets[kind].runEnd = sprintEndFrames; assets[kind].walkStart = walkStartFrames; assets[kind].walkEnd = walkEndFrames; });
assets.cyclops.down = ['Down_00.png','Down_01.png','Down_02.png','Down_03.png'];
assets.wolverine.down = ['Down.png'];
...
function preloadFighterAssets(kind) {
  ...
  const images = [];
  states.forEach(state => assets[kind][state].forEach(frame => images.push(imageFor(root + frame, defaultRoot(kind) + frame))));
...
let projectiles = [];
const CYCLOPS_BASIC_REACH = 175;
const KNOCKDOWN_INVULNERABLE = 1200;
const DOWN_HOLD_MS = 700;
...
class Fighter {
  ...
  animate(dt) { this.frameClock += dt; this.stateTime += dt; const rate = this.state === 'idle' ? 115 : this.state === 'special' ? 42 : 82; if (this.frameClock > rate) { this.frameClock = 0; this.frame += 1; const frames = this.state === 'attack' && assets[this.kind].attackStages ? assets[this.kind].attackStages[this.attackStage - 1] : assets[this.kind][this.state] || assets[this.kind].idle; if (this.frame >= frames.length) { if (this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special')) { this.frame = frames.length - 1; } else { const next = ONE_SHOT_NEXT[this.state]; if (next) this.setState(next); else this.frame = 0; } } } }
  takeHit(damage, push) { if (this.invulnerable > 0 || this.health <= 0) return; const knockdown = damage >= 10 || this.health - damage <= 0; this.health = Math.max(0, this.health - damage); this.meter = Math.min(3, this.meter + .35); this.hitFlash = 130; this.setState(knockdown ? 'kd' : 'hit'); if (knockdown) this.invulnerable = KNOCKDOWN_INVULNERABLE; this.x += push; }
  attack(type, opponent) { const airborne = !this.grounded; const canStartFromWalk = type === 'sprintAttack' && this.state === 'walk'; if (this.attackCooldown > 0 || (this.state !== 'idle' && !canStartFromWalk) || (airborne && type !== 'airPower') || (!airborne && type === 'airPower')) return false; const cost = type === 'special' ? 3 : type === 'power' || type === 'airPower' ? 1 : 0; if (this.meter < cost) return false; this.meter -= cost; this.rangedBasic = type === 'attack' && this.kind === 'cyclops' && !!opponent && Math.abs(opponent.x - this.x) >= CYCLOPS_BASIC_REACH; this.attackStage = type === 'attack' && !this.rangedBasic && this.comboTimer > 0 ? Math.min(4, this.chainStage + 1) : 1; this.projectileSpawned = false; this.setState(type); if (type === 'risingAttack') this.vy = -12; this.strikeConfirmed = false; this.attackCooldown = type === 'attack' || type === 'sprintAttack' || type === 'risingAttack' ? 430 : type === 'airPower' || type === 'power' ? 640 : 1100; return true; }
  update(dt, opponent, inputKeys = keys, inputPressed = pressed) {
    ...
    if (this.health <= 0) { if (!['kd','down','downgetup'].includes(this.state)) this.setState('kd'); this.animate(dt); return; }
    if (['hit','kd','downgetup','land'].includes(this.state)) { this.animate(dt); return; }
    if (this.state === 'down') { if (this.stateTime > DOWN_HOLD_MS) this.setState('downgetup'); this.animate(dt); return; }
    if (['attack','airAttack','airPower','sprintAttack','risingAttack','power','special'].includes(this.state)) { if (!this.grounded) { this.vy += .75 * dt / 16; this.y += this.vy * dt / 16; if (this.y >= FLOOR) { this.y = FLOOR; this.vy = 0; } } this.checkStrike(opponent); if (this.kind === 'cyclops' && this.state === 'attack' && this.rangedBasic && !this.projectileSpawned && this.stateTime >= 230) { projectiles.push(new Projectile(this, 'attack1')); this.projectileSpawned = true; } if (this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special') && !this.projectileSpawned && this.stateTime >= (this.state === 'special' ? 0 : 260)) { projectiles.push(new Projectile(this, this.state)); this.projectileSpawned = true; this.strikeConfirmed = true; } if (this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special') && this.projectileSpawned && !projectiles.some(projectile => projectile.owner === this && projectile.type === this.state)) { this.setState('idle'); this.animate(dt); return; } this.animate(dt); return; }
    if (this.state === 'dodge') { this.invulnerable = 80; this.x -= this.facing * 5; this.x = Math.max(90, Math.min(W - 90, this.x)); this.animate(dt); return; }
    if (!this.grounded) { if (inputPressed.has('x') && this.state === 'jump') { this.attackStage = 1; this.projectileSpawned = false; this.strikeConfirmed = false; this.setState('airAttack'); this.vy = Math.min(this.vy, 1); } if (inputPressed.has('a') && this.state === 'jump') this.attack('airPower', opponent); if (this.state === 'jump') { if (inputKeys.has('ArrowLeft')) this.x -= 4.2 * dt / 16; if (inputKeys.has('ArrowRight')) this.x += 4.2 * dt / 16; this.x = Math.max(90, Math.min(W - 90, this.x)); } this.vy += .75 * dt / 16; this.y += this.vy * dt / 16; if (this.y >= FLOOR) { this.y = FLOOR; this.vy = 0; if (this.state === 'jump') this.setState('land'); } if (this.state === 'jump' || this.state === 'land') this.animate(dt); return; }
    if (this.cpu) this.ai(opponent, dt); else this.playerInput(dt, inputKeys, inputPressed, opponent);
    this.x = Math.max(90, Math.min(W - 90, this.x));
    this.facing = opponent.x >= this.x ? 1 : -1;
    this.animate(dt);
  }
  playerInput(dt, inputKeys, inputPressed, opponent) { const left = inputKeys.has('ArrowLeft'), right = inputKeys.has('ArrowRight'), moving = left || right; if (inputPressed.has('ArrowDown')) { this.setState('dodge'); this.invulnerable = 300; return; } if (inputPressed.has('ArrowUp') && inputPressed.has('x')) { if (this.attack('risingAttack', opponent)) { this.y -= 2; return; } } if (inputPressed.has('x')) { if (this.attack(moving ? 'sprintAttack' : 'attack', opponent)) return; } if (inputPressed.has('a') && this.attack('power', opponent)) return; if (inputPressed.has('s') && this.attack('special', opponent)) return; if (moving) { const sprinting = inputKeys.has('Shift'); this.x += (right ? 1 : -1) * (sprinting ? 7.4 : 4.2) * dt / 16; const loop = sprinting ? 'run' : 'walk', startState = sprinting ? 'runStart' : 'walkStart'; if (this.state === 'idle' || this.state === 'walkEnd' || this.state === 'runEnd') this.setState(startState); else if (this.state !== loop && this.state !== startState) this.setState(loop); } else if (this.state === 'walk' || this.state === 'run') { this.setState(this.state === 'run' ? 'runEnd' : 'walkEnd'); } else if (this.state !== 'walkEnd' && this.state !== 'runEnd') { this.setState('idle'); } if (inputPressed.has('ArrowUp')) { this.vy = -15; this.y -= 2; this.setState('jump'); } }
  ai(opponent, dt) { const profile = this.aiProfile; const distance = Math.abs(opponent.x - this.x); this.aiDecisionClock -= dt; if (profile.reaction && opponent.state === 'attack' && distance < 210 && Math.random() < profile.reaction * dt / 16) { this.setState('dodge'); this.invulnerable = 300; return; } if (distance > profile.idealRange) { this.x += this.facing * profile.approachSpeed * dt / 16; if (this.state === 'idle' || this.state === 'walkEnd') this.setState('walkStart'); else if (this.state !== 'walk' && this.state !== 'walkStart') this.setState('walk'); return; } if (this.attackCooldown <= 0 && this.aiDecisionClock <= 0 && Math.random() < profile.attackChance * dt / 16) { this.aiDecisionClock = 260 / (profile.approachSpeed / 1.2); if (this.meter >= 3 && Math.random() < profile.specialChance) this.attack('special', opponent); else if (this.meter >= 1 && Math.random() < .3 + profile.specialChance) this.attack('power', opponent); else this.attack('attack', opponent); return; } if (this.state === 'walk') this.setState('walkEnd'); else if (this.state !== 'walkEnd') this.setState('idle'); }
  checkStrike(opponent) { if (this.strikeConfirmed) return; const isCyclopsBeam = this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special'); if (isCyclopsBeam) return; const windows = this.state === 'airAttack' || this.state === 'airPower' ? [140, 420] : this.state === 'risingAttack' ? [160, 420] : this.state === 'sprintAttack' ? [150, 360] : this.state === 'attack' ? [140, 330] : this.state === 'power' ? [260, 560] : [480, 900]; const [start, end] = windows; if (this.stateTime < start || this.stateTime > end) return; const reach = this.state === 'airAttack' || this.state === 'airPower' ? 170 : this.state === 'risingAttack' ? 145 : this.state === 'sprintAttack' ? 210 : this.state === 'special' ? 260 : this.state === 'power' ? 210 : CYCLOPS_BASIC_REACH; const damage = this.state === 'airPower' ? 12 : this.state === 'airAttack' ? 8 : this.state === 'risingAttack' ? 10 : this.state === 'sprintAttack' ? 9 : this.state === 'special' ? 24 : this.state === 'power' ? 12 : 6; if (Math.abs(opponent.x - this.x) < reach && Math.abs(opponent.y - this.y) < 180) { opponent.takeHit(damage, this.facing * 28); this.meter = Math.min(3, this.meter + .6); if (['attack','airAttack','airPower','sprintAttack','risingAttack'].includes(this.state)) { this.combo = Math.min(10, this.combo + 1); this.chainStage = this.attackStage; this.comboTimer = 900; } this.strikeConfirmed = true; } }
  draw() { const path = framePath(this, this.state, this.frame); const image = imageFor(path, fallbackFramePath(this, this.state, this.frame)); if (image.complete && image.naturalWidth) this.lastImage = image; if (!this.lastImage) return; ctx.save(); if (this.hitFlash > 0) ctx.globalAlpha = .55; ctx.translate(this.x, this.y); ctx.scale(this.facing * FIGHTER_SCALE, FIGHTER_SCALE); ctx.drawImage(this.lastImage, -128, -224, 256, 256); ctx.restore(); }
}
...
const skyGradient = ctx.createLinearGradient(0, 0, 0, FLOOR);
...
function start() {
  if (gameMode === 'online' && !onlineReady) { lobbyStatus.textContent = 'Wait for both players to join the lobby.'; return; }
  if (loadingMatch) return;
  loadingMatch = true;
  reset();
  const opponentKind = selectedKind === 'cyclops' ? 'wolverine' : 'cyclops';
  ...
  Promise.all([waitForImages(preloadFighterAssets(selectedKind)), waitForImages(preloadFighterAssets(opponentKind))]).then(() => {
    ...
    footerState.textContent = gameMode === 'cpu' ? `ROUND ACTIVE // ${selectedDifficulty.toUpperCase()} CPU` : 'ROUND ACTIVE // ONLINE 1V1';
