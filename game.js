...
class Fighter {
  ...
  attack(type) { if (this.attackCooldown > 0 || this.state !== 'idle' || !this.grounded) return false; const cost = type === 'special' ? 3 : type === 'power' ? 1 : 0; if (this.meter < cost) return false; this.meter -= cost; this.setState(type); this.strikeConfirmed = false; this.attackCooldown = type === 'attack' ? 370 : type === 'power' ? 640 : 1100; return true; }
  update(dt, opponent) {
    ...
    if (this.state === 'attack' || this.state === 'power' || this.state === 'special') { this.checkStrike(opponent); this.animate(dt); return; }
  ...
  checkStrike(opponent) { if (this.strikeConfirmed) return; const windows = { attack: [140, 330], power: [260, 560], special: [480, 900] }; const [start, end] = windows[this.state]; if (this.stateTime < start || this.stateTime > end) return; const reach = this.state === 'special' ? 380 : this.state === 'power' ? 330 : 175; if (Math.abs(opponent.x - this.x) < reach && Math.abs(opponent.y - this.y) < 180) { const damage = this.state === 'special' ? 24 : this.state === 'power' ? 12 : 6; opponent.takeHit(damage, this.facing * 28); this.meter = Math.min(3, this.meter + .6); this.combo += 1; this.strikeConfirmed = true; } }
