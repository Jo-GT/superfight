...
class Fighter {
  ...
  update(dt, opponent, inputKeys = keys, inputPressed = pressed) {
    ...
    if (!this.grounded) { if (inputPressed.has('x') && this.state === 'jump') { this.attackStage = 1; this.projectileSpawned = false; this.strikeConfirmed = false; this.setState('airAttack'); this.vy = Math.min(this.vy, 1); } if (inputPressed.has('a') && this.state === 'jump') this.attack('airPower'); if (this.state === 'jump') { if (inputKeys.has('ArrowLeft')) this.x -= 4.2 * dt / 16; if (inputKeys.has('ArrowRight')) this.x += 4.2 * dt / 16; this.x = Math.max(90, Math.min(W - 90, this.x)); } this.vy += .75 * dt / 16; this.y += this.vy * dt / 16; if (this.y >= FLOOR) { this.y = FLOOR; this.vy = 0; } if (this.state === 'jump') this.animate(dt); return; }
