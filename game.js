...
class Fighter {
  ...
  animate(dt) { this.frameClock += dt; this.stateTime += dt; const rate = this.state === 'idle' ? 115 : this.state === 'special' ? 42 : 82; if (this.frameClock > rate) { this.frameClock = 0; this.frame += 1; const frames = this.state === 'attack' && assets[this.kind].attackStages ? assets[this.kind].attackStages[this.attackStage - 1] : assets[this.kind][this.state] || assets[this.kind].idle; if (this.frame >= frames.length) { const next = ONE_SHOT_NEXT[this.state]; if (next) this.setState(next); else this.frame = 0; } } }
  ...
  update(dt, opponent, inputKeys = keys, inputPressed = pressed) {
    ...
    if (['attack','airAttack','airPower','sprintAttack','risingAttack','power','special'].includes(this.state)) { if (!this.grounded) { this.vy += .75 * dt / 16; this.y += this.vy * dt / 16; if (this.y >= FLOOR) { this.y = FLOOR; this.vy = 0; } } this.checkStrike(opponent); if (this.kind === 'cyclops' && this.state === 'attack' && this.rangedBasic && !this.projectileSpawned && this.stateTime >= 230) { projectiles.push(new Projectile(this, 'attack1')); this.projectileSpawned = true; } if (this.kind === 'cyclops' && (this.state === 'power' || this.state === 'special') && !this.projectileSpawned && this.stateTime >= (this.state === 'special' ? 0 : 260)) { projectiles.push(new Projectile(this, this.state)); this.projectileSpawned = true; this.strikeConfirmed = true; } this.animate(dt); return; }
